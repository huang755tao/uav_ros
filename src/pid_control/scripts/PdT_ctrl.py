...
if __name__ == "__main__":
    ...
    # 实例化控制器
    # 双闭环的参数
    pid_ctrl = PidControl(kp_pos=np.array([0.6, 0.6, 1.2]),
                ki_pos=np.array([0.001, 0.001, 0.002]),
    ...
    ft_ctrl = FTBC(dt=dt,                     
                   ...
                   p_eta= 0.2,
                   T_eta= 10.)
    ft_ctrl.k11 = np.array([8., 8., 12.5])
    ft_ctrl.k21 = np.array([30., 30., 40.])
    ...
    pdt_ctrl.k11 = np.array([10., 12., 15.])
    ...
    circle_gen = TrajectoryGenerator(
        ...
        dt=dt,
        amplitudes=(1., 1., 0.0),  # X/Y/Z轴振幅
        freqs=(0.125, 0.125, 0.1)         # X/Y/Z轴频率
    )
    ...
    circle_gen.p0 = np.array([0, 0, 1.5])
    ...
    while (not rospy.is_shutdown()) and (sim_t < simulation_time):  
        ...
        # ref_state = np.zeros(9)
        # ref_state[0:3]=np.array([0.5, 0.8, 1.5])
        ...
        pdt_ctrl.update(state=uav_state, ref_state=ref_state, obs= 0.2*(obs.clip(-np.array([0.15, 0.25, 0.5]), np.array([0.15, 0.25, 0.5]))))
